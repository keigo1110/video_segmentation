...
def initialize_input_source(config: Config) -> Dict[str, Any]:
    ...
    if config.input_type == "video":
        ...
        image_files = []
        # 指定されたパターンを優先する
        if config.image_pattern and config.image_pattern != '*.png': # デフォルト以外が指定された場合
             search_path = os.path.join(config.input_path, config.image_pattern)
             image_files = sorted(glob.glob(search_path))
             if not image_files:
                 logger.warning(f"指定されたパターン {search_path} に一致する画像がありません。デフォルトの拡張子で再試行します。")

        # デフォルトの拡張子 (.png, .jpg, .jpeg) で検索（指定パターンで見つからない場合 or デフォルトの場合）
        if not image_files:
             default_patterns = ["*.png", "*.jpg", "*.jpeg"]
             for pattern in default_patterns:
                 search_path = os.path.join(config.input_path, pattern)
                 image_files.extend(glob.glob(search_path))
             image_files = sorted(list(set(image_files))) # 重複除去とソート
        # --- 修正ここまで ---

        if not image_files:
            # 再試行後もファイルが見つからない場合
            raise ValueError(
                f"フォルダ {config.input_path} 内に画像ファイルが見つかりません (試行パターン: {config.image_pattern} および *.png, *.jpg, *.jpeg)"
            )
